...
const userRef = collection(myDatabase, "testing");
...
.then((snapshot)=>{
    ...
    let loginData = [];
    snapshot.docs.forEach((doc)=>{
        loginData.push({ ...doc.data(), id: doc.id});
    })
    ...
    const {register, handleSubmit, reset, formState: { errors }} = useForm();
		const onSubmit = (data) => {
            ...
            addDoc(userRef, {
                ...
                login_time: currentHour 
            }).
            then(()=>{
                console.log("currentHour:", currentHour);
                if (currentHour >= currentHour+72){
                    console.log("cannotplay for 72 hours")
                } else {
                    console.log("user is able to play")
                }
            })
            .catch((err) => {
                console.log(err.message);
        ...
        <section className="form_holster">
			...
			<form onSubmit={handleSubmit(onSubmit)} className="fieldset" alt="https://unsplash.com/photos/brown-wooden-lounge-chairs-near-pool-surrounded-by-palm-trees-vmIWr0NnpCQ">
				...
				<section className="form_category main_address_section">
                    ...
                    {isFormSubmitted && (
                        <Link to="/legal">Continue to Legal Page</Link>
                        )}
                    </section>
